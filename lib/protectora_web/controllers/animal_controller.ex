defmodule ProtectoraWeb.AnimalController do
  ...
  defp process_images_inner(imaxes, id, nombres, n) do
    case imaxes do
        ...
        [h | t] ->  "data:image/gif;base64," <> raw = h
                    ...
                    process_images_inner(t, id, ["/animais/" <> id <> Integer.to_string(n) <> ".jpg" | nombres], n + 1)
      end
  end
  ...
  def create(conn, %{"animal" => animal_params,  "imaxes" => imaxes}) do
    ...
    with {:ok, animal_created} <- Animais.create_animal(animal_params, fn animal_result -> process_images(imaxes, animal_result.id, [], 0) end) do
  ...
  defp update_imaxes(animal, imaxes) do
    case imaxes do
      [] -> animal
      list -> Enum.each(animal.imaxe_animal, fn el -> File.rm!(Path.join(["priv/static", el.path_imaxe])) end)
  ...
  def update(conn, %{"id" => id, "animal" => animal_params, "imaxes" => imaxes}) do
    ...
    with {:ok, animal_updated} <- Animais.update_animal(animal, animal_params,  fn animal_result -> update_imaxes(animal_result, imaxes) end) do
      case animal_updated do
        {:error, changeset} -> conn
                                |> put_status(:unprocessable_entity)
                                |> put_view(ProtectoraWeb.ChangesetView)
                                |> render("error.json", changeset: changeset)
        {_, updated} -> render(conn, "show.json", animal: updated)
